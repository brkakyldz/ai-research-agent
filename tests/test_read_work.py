...
@pytest.fixture
async def one_bulletin(session: AsyncSession) -> Run:
...
async def test_the_shell_asks_for_the_latest_run_once(
    client: TestClient, engine: AsyncEngine, one_bulletin: Run
) -> None:
    """`advice.last` and `latest_finished_run()` were the same query, asked twice
    on every page in the app - and being two of them, they could disagree."""
...
async def test_the_full_list_counts_its_tags_once(
    ...
    """`?all=1` needs the tags of the whole run and of the ranked stories, and
    read every `tags_json` in the run twice to get them."""
    with counted(engine, "summaries") as statements:
        assert client.get("/?all=1").status_code == 200
    tag_scans = [s for s in statements if s.startswith("SELECT summaries.tags_json")]
...
async def test_the_filter_still_decides_membership_exactly(
    client: TestClient, one_bulletin: Run
) -> None:
...
async def test_the_ranker_names_its_sources_in_one_query(
    session: AsyncSession, engine: AsyncEngine, one_bulletin: Run
) -> None:
    """Two `session.get` calls per summary is a hundred and eighty round trips
    on a ninety-story day, to read one column of one table."""
    from ainews.pipeline import graph as graph_module

    article_ids = list((await session.execute(select(Article.id))).scalars())
    assert len(article_ids) == 3

    with counted(engine, "articles") as statements:
        meta = await graph_module._source_meta(article_ids)
    assert len(statements) == 1, statements
    assert all(name == "OpenAI" and weight == 2.0 for name, weight in meta.values())
