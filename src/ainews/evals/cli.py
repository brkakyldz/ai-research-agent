...
# Every subcommand takes the same reference, because they all measure the same
# object: a published bulletin (ADR 0030). A day is what a person has in mind and
# resolves to that day's current version - the page they would be looking at.
BULLETIN_HELP = (
    "bulletin to measure: 'latest', a numeric id, a day like 2026-09-08, "
    "or a day and language like 2026-09-08:tr"
)
...
def add_eval_parser(sub: argparse._SubParsersAction) -> None:  # type: ignore[type-arg]
    ...
    record = evals.add_parser("record", help="write a bulletin's output to a JSON fixture (no key)")
    record.add_argument("--bulletin", default="latest", help=BULLETIN_HELP)
    record.add_argument(
        "--out", type=Path, default=None, help="directory (default: tests/fixtures/runs)"
    )

    judge = evals.add_parser(
        "judge", help="sampled grounding judge on one bulletin (spends money; estimates first)"
    )
    judge.add_argument("--bulletin", default="latest", help=BULLETIN_HELP)
    ...
    stability.add_argument("--bulletin", default="latest", help=BULLETIN_HELP)
    ...
    report.add_argument(
        "--bulletin",
        default=None,
        help="one bulletin only; the spend totals keep the window. " + BULLETIN_HELP,
    )
...
async def _record(ref: str, out: Path | None) -> int:
    from ainews.evals.record import record_bulletin, write_fixture
    from ainews.pipeline.runner import resolve_bulletin

    await init_db(get_engine())
    async with session_scope() as session:
        bulletin_id = await resolve_bulletin(session, ref)
        fixture = await record_bulletin(session, bulletin_id)
    path = write_fixture(fixture, out)
    print(f"recorded bulletin {fixture['day']} ({len(fixture['stories'])} stories) to {path}")
    return 0


async def _judge(args: argparse.Namespace) -> int:
    from ainews.evals.judge import CostGuard, format_calibration, judge_bulletin, judge_labelled
    ...
    try:
        async with session_scope() as session:
            if args.labelled:
                ...
                return 0
            bulletin_id = await resolve_bulletin(session, args.bulletin)
    ...
    print(
        f"bulletin {bulletin_id}: {report.n_passed} passed, {report.n_failed} failed, "
...
async def _stability(args: argparse.Namespace) -> int:
    ...
    async with session_scope() as session:
        ...
        report = await rank_stability(
            session, bulletin_id, times=args.times, seed=args.seed, model=args.model
        )
    print(
        f"bulletin {bulletin_id}: {report.times} shuffled rank calls on {report.model} "
        f"against the published order, mean tau {report.tau:.3f}, "
        f"mean top-N Jaccard {report.jaccard:.3f}, ~${report.est_cost_usd:.4f}"
        + (f", {report.n_fallback} call(s) failed" if report.n_fallback else "")
    )
    return 0
...
async def _report(args: argparse.Namespace) -> int:
    ...
    await init_db(get_engine())
    async with session_scope() as session:
        chosen = await resolve_bulletin(session, args.bulletin) if args.bulletin else None
        report = await build_report(session, since_days=parse_since(args.since), bulletin_id=chosen)
...
async def run_eval(args: argparse.Namespace) -> int:
    try:
        if args.eval_command == "record":
            return await _record(args.bulletin, args.out)
